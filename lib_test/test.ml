...
let with_temp_stream f () =
  ...
  let lwt =
    Cf_lwt.RunLoop.run_thread (fun runloop ->
        ...
        if not (Fsevents_lwt.start watcher) then
          print_endline "failed to start FSEvents stream")
    >>= fun runloop ->
    f dir >>= fun f ->
    f watcher >>= fun () ->
...
module Event = struct
  ...
  let fail_event remaining event_type =
    Alcotest.fail
      ("expected "
      ^ event_type
      ^ " event but current event is:\n"
      ^ remaining.path
      ^ "\n"
      ^ to_string_one_line remaining.flags)

  let has_create remaining =
    if remaining.flags.item_created then
  ...
  let has_remove remaining =
    if remaining.flags.item_removed then
  ...
  let has_modify remaining =
    if remaining.flags.item_modified then
  ...
  let has_meta_modify remaining =
    if remaining.flags.item_inode_meta_mod then
      {
        remaining with
        flags = { remaining.flags with item_inode_meta_mod = false };
      }
    else fail_event remaining "ItemInodeMetaMod"

  let has_change_owner remaining =
    if remaining.flags.item_change_owner then
      {
        remaining with
  ...
  let rec check_one remaining = function
    | Create path ->
        ...
        has_create remaining
    | Remove path ->
        check_path remaining path;
        has_remove remaining
    | Modify path ->
        check_path remaining path;
        has_modify remaining
    | MetaModify path ->
        check_path remaining path;
        has_meta_modify remaining
    | ChangeOwner path ->
        check_path remaining path;
  ...
  let end_of_stream stream =
    Lwt_stream.is_empty stream >>= fun is_empty ->
...
module FileMod = struct
  ...
  let noop dir = return Event.(expect [ DirType (Create dir) ])

  let create dir =
    let path = dir / "create_file_test" in
    Lwt_unix.(openfile path [ O_CREAT ] 0o600) >>= fun fd ->
    Lwt_unix.close fd >>= fun () ->
    return Event.(expect [ DirType (Create dir); FileType (Create path) ])
  ...
  let create_remove dir =
    ...
    return
      Event.(
        expect [ DirType (Create dir); FileType (Create path); Remove path ])

  let modify dir =
    let file = "modify_file_test" in
    let path = dir / file in
    Lwt_unix.(openfile path [ O_CREAT; O_WRONLY ] 0o600) >>= fun fd ->
    Lwt_unix.write fd (Bytes.of_string file) 0 (String.length file)
    >>= fun _written ->
    Lwt_unix.close fd >>= fun () ->
    return
      Event.(
        expect [ DirType (Create dir); FileType (Create path); Modify path ])

  let chmod dir =
    Lwt_unix.chmod dir 0o750 >>= fun () ->
    return Event.(expect [ DirType (Create dir); ChangeOwner dir ])
...
let tests = [ ("FileMod", FileMod.tests) ];;
